...
async def handle_webhook(request, app: Application):
...
async def create_application() -> Application:
    ...
    logger.info("🚀 Создание приложения Heaven & Bloody Stats Bot...")
...
async def main():
    ...
    if RENDER_EXTERNAL_URL:
        ...
        # Создаем хэндлер для веб-хука с привязкой к приложению
        async def webhook_handler(request):
            return await handle_webhook(request, app)
        
        web_app.router.add_post('/webhook', webhook_handler)
    else:
        logger.warning("⚠️ RENDER_EXTERNAL_URL не установлен, веб-хук не настроен")
        # Если нет URL для веб-хука, запускаем в polling режиме
    ...
    # Регистрация маршрутов для проверки здоровья
    ...
    # Запускаем периодическую проверку API
    ...
    try:
        ...
        if RENDER_EXTERNAL_URL:
            ...
            try:
                ...
                logger.info("✅ Веб-хук отключен")
            except Exception as e:
                logger.error(f"Ошибка при отключении веб-хука: {e}")
        else:
